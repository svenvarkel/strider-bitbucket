...
function parseCommitData(data) {
  ...
  if (!data.commits || !data.commits.length) {
    console.log('possible error of retrieval of data from the api as data seems to be empty', data);
    return { notInteresting: true }
  }

  var commit = data.commits[data.commits.length - 1] || {};
  var rawAuthor = commit.raw_author || '';
  var author = parseAuthor(rawAuthor);
...
function startCommitJob(payload, project, emitter, done) {
  ...
  console.log('commit payload', payload, project, emitter);
